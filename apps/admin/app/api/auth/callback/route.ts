...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    if (capturedFence === null)
      return redirect('/admin/login?error=callback_expired');
    ...
    const sessionId = data.session
      ...
      : null;
    if (error || !data.session || !sessionId)
      return redirect('/admin/login?error=callback_failed');
