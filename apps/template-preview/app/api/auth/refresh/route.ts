...
function failureResponse(id: string, status: 401 | 429 | 503, clear: boolean, secure: boolean): NextResponse {
  ...
  if (clear) terminalClearAuthSessionCookies({ writer: response.cookies as unknown as AuthCookieWriter, secure, prefix: 'consumer' });
  return response;
}
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    writeAuthSessionCookies({ writer: response.cookies as unknown as AuthCookieWriter, session: data.session, secure, prefix: 'consumer', csrfToken: request.cookies.get(names.csrf)?.value });
    return response;
  } catch {
    return failureResponse(id, 503, false, secure);
  }
