...
import { callbackReturnTo } from '@kit/account-auth-nextjs';
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    if (capturedFence === null) return redirect('/login?error=callback_expired');
    ...
    if (error || !data.session || !sessionId) return redirect('/login?error=callback_failed');
    const response = NextResponse.redirect(new URL(returnTo, request.url));
    response.headers.set('Cache-Control', 'no-store');
    response.headers.set('X-Request-Id', id);
    writeAuthSessionCookies({ writer: response.cookies as unknown as AuthCookieWriter, session: data.session, secure: process.env.NODE_ENV === 'production', prefix: 'consumer' });
    writeLoginAcknowledgement({ writer: response.cookies as unknown as AuthCookieWriter, secure: process.env.NODE_ENV === 'production', sessionId, loginFence: capturedFence, prefix: 'consumer' });
