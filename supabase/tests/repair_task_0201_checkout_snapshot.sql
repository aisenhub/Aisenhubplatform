...
select ok(
  pg_get_functiondef('private.subscription_checkout_payment_facts(private.account_context, uuid)'::regprocedure)
    like '%provider_external_plan_id_snapshot%'
    and pg_get_functiondef('private.subscription_checkout_payment_facts(private.account_context, uuid)'::regprocedure)
      not like '%billing_provider_products%',
  'payment-link facts use the Checkout snapshot instead of the current mapping'
);
