...
select has_table('private', 'account_retention_policy', 'retention policy exists');
...
select throws_ok(
  ...
  'retention anonymization fails closed until a legal or financial policy is configured'
);
select ok(pg_get_functiondef('private.account_retention_candidates(timestamptz, uuid, integer)'::regprocedure) not like '%30 days%', 'retention does not guess a fixed default period');
