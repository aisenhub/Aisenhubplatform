-- TASK-0102: close the shared entitlement write-entry lifecycle gap.
-- Billing and Admin callers bypass account_principal, so the platform status
-- must be checked again here while holding the platform row lock.
...
begin
  ...
  -- FOR SHARE serializes this write with admin platform disable/update.  The
  -- lock is acquired before the account lock, matching the domain lock order.
