...
function database(valuesSeen: unknown[][], duplicate = false) {
  return {
    async begin<T>(callback: (transaction: {
...
function request(body: string, eventKey = 'provider-event-1') {
  return new Request('http://local/functions/v1/billing-webhook/webhooks/afdian', {
    method: 'POST',
    headers: {
...
Deno.test('billing webhook persists only hash and enqueue result, with duplicate ACK support', async () => {
  ...
  const first = await handleBillingWebhookRequest(
    request(JSON.stringify({ order_no: 'order-1', secret_payload: 'do-not-store' })),
    {
      providerAccountId,
      database: database(valuesSeen),
      verifySignature: async () => true,
  ...
  assert(!valuesSeen.some((values) => values.some((value) => value === 'do-not-store')));
