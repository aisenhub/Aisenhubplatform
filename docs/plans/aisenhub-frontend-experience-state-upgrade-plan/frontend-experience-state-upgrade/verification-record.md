...
| 当前产品实现基线（main） | `c9c19f4` | 已验证 | 资源 loader generation/epoch、FE-V 代表性错误/竞态矩阵、Files/Policy/Settings 412/429/503/202、Admin MFA Next Image；最新 FE-V Local 浏览器回归为 `c9c19f4`，FE-D02 批次边界基线仍为 `54ff797`，origin/main 已核对一致 |
...
- 被验证 commit：`a5314fd742cc3b51b4fb3971bc48b0103d8e8481`（产品实现基线仍为 `54ff79727dd0b7ea734822d23db2c9b58d6fe4ea`）
...
## VR-0013 — Admin 错误文案与技术详情隔离 Local 复跑

- 日期：2026-09-10
- 阶段：T16-R2 / FE-V 状态与错误文案 supporting evidence
- 被验证 commit：`868e069c1e9631cad09021598bc6070db23a694e`（FE-D02 批次边界产品基线仍为 `54ff79727dd0b7ea734822d23db2c9b58d6fe4ea`）
...
- 命令：`pnpm --filter admin build`；`pnpm run test:e2e:t16-r2`；`pnpm lint`；`pnpm contracts:check`；`pnpm run docs:check`；目标文件 `oxfmt --check`；`git diff --check`。
- 结果：Admin build、lint、OpenAPI contract、docs consistency、目标格式和 diff 检查均 PASS；T16-R2 退出码 `0`，17 项均 `PASS`。新增 `adminErrorCopyMatrix` 对 429 `RATE_LIMITED`、503 `AUTHORIZATION_UNAVAILABLE`、409 `IDEMPOTENCY_CONFLICT` 验证中文主提示，内部 code 只保留在可展开技术详情；不把原始 code 直接作为用户摘要。
- 未覆盖：迟到响应、完整 409/412/429/503/202 故障注入、Files/Settings 全状态矩阵、Hosted/Staging/生产与正式发布；本记录只关闭新增的 Local Admin 文案边界，不升级 FE-V 总体状态。
- 该结果是否仍覆盖当前代码：是；`origin/main` 与 `origin/codex/frontend-plan-r1` 已核对均为该 commit。

---

## VR-0014 — Files 状态、预算与删除 unknown Local 复跑

- 日期：2026-09-10
- 阶段：T16-R2 / FE-V09 / Phase 05 supporting evidence
- 被验证 commit：`bb65f1840e8b3f67804a0f8ca3bc36cb9ad4c921`（产品实现基线为 `868e069c1e9631cad09021598bc6070db23a694e`，FE-D02 批次边界基线为 `54ff79727dd0b7ea734822d23db2c9b58d6fe4ea`）
- 环境：Windows 10 家庭中文版；Node v24.19.0；pnpm 11.18.0；Next 16.3.0；Supabase CLI 2.111.0；Local Supabase；Playwright headless Chrome；Admin production build。
- 命令：`pnpm run test:e2e:t16-r2`；此前同一代码基线的 Admin build、lint、OpenAPI、docs 和目标格式检查结果见 VR-0013。
- 结果：退出码 `0`；`filesStateMatrix` 通过策略先返回、文件列表延迟 1.2 秒、active/receiving/storing/deleting/unknown/deleted 状态映射、预算超限提示、下载/删除行级禁用边界；模拟 DELETE 网络中断后只发一次请求，权威状态检查保持 unknown，不自动重发。
- 未覆盖：Files 下载失败/MFA、Policy 保存 MFA/409、完整 202/迟到响应、Origins/Keys 全状态、Hosted/Staging/生产和正式发布；本记录只补 Local Files 状态与删除恢复证据，不升级 FE-V 总体状态。
- 该结果是否仍覆盖当前代码：是；`origin/main` 与 `origin/codex/frontend-plan-r1` 已核对均为该 commit。

---

## VR-0015 — Settings Origins/Platform Key 生命周期 Local 复跑

- 日期：2026-09-10
- 阶段：T16-R2 / Phase 05 / FE-V Settings supporting evidence
- 被验证 commit：`68d565a992462fe5431b0947b57874c24889086d`（产品实现基线为 `868e069c1e9631cad09021598bc6070db23a694e`，FE-D02 批次边界基线为 `54ff79727dd0b7ea734822d23db2c9b58d6fe4ea`）
- 环境：Windows 10 家庭中文版；Node v24.19.0；pnpm 11.18.0；Next 16.3.0；Supabase CLI 2.111.0；Local Supabase；Playwright headless Chrome；Admin production build。
- 命令：`pnpm run test:e2e:t16-r2`；同一 runner 的 Admin build、lint、OpenAPI、docs 和目标格式检查结果见 VR-0013/VR-0014。
- 结果：退出码 `0`；`settingsLifecycleMatrix` 通过无效 Origin URL 字段校验、有效 Origin 创建及列表刷新；Platform Key secret 只在一次性面板展示，确认已知 secret 后从 DOM 脱离，完成部署确认并执行撤销；T16-R2 当前共 19 项 Local 浏览器断言均为 `PASS`。
- 未覆盖：Settings 的 MFA/409/412/429/503、网络 unknown/迟到响应、完整 Origins/Keys 失败恢复矩阵，以及 Hosted/Staging/生产和正式发布；本记录只补 Local 生命周期证据，不升级 FE-V 总体状态。
- 该结果是否仍覆盖当前代码：是；`origin/main` 与 `origin/codex/frontend-plan-r1` 已核对均为该 commit。

---

## VR-0016 — Files 下载与 Policy 保存故障恢复 Local 复跑

- 日期：2026-09-10
- 阶段：T16-R2 / Phase 05 / FE-V Files supporting evidence
- 被验证 commit：`1a8cd5e49928537a5e3163bf0d758dbe41a7aff7`（产品实现基线与 runner 扩展同一提交；FE-D02 批次边界基线为 `54ff79727dd0b7ea734822d23db2c9b58d6fe4ea`）
- 环境：Windows 10 家庭中文版；Node v24.19.0；pnpm 11.18.0；Next 16.3.0；Supabase CLI 2.111.0；Local Supabase；Playwright headless Chrome；Admin production build。
- 命令：`pnpm --filter admin typecheck`；`pnpm --filter admin build`；`pnpm run test:e2e:t16-r2`；目标文件 `oxfmt --check`；`git diff --check`。
- 结果：Admin typecheck/build 通过；浏览器 runner 退出码 `0`，共 19 项均为 `PASS`。`filesStateMatrix` 额外验证下载 `STORAGE_UNAVAILABLE` 的可操作中文错误与折叠技术详情、`MFA_REQUIRED` 进入下载 step-up 且不自动重放、Policy 保存 `RECENT_MFA_REQUIRED` 后显式完成 MFA，再对 `IDEMPOTENCY_CONFLICT` 展示冲突并不自动重试。新增父级状态修复保证 step-up 期间平台上下文与 Files 页面不被卸载。
- 未覆盖：迟到响应、完整 409/412/429/503/202 故障注入、Settings 全状态故障恢复、Hosted/Staging/生产和正式发布；本记录只补 Local Files/Policy 边界，不升级 FE-V 总体状态。
- 该结果是否仍覆盖当前代码：是；`origin/main` 与 `origin/codex/frontend-plan-r1` 已核对均为该 commit。
...
## VR-0018 — FE-V 资源竞态与错误矩阵 Local 复跑

- 日期：2026-09-10。
- 阶段：T16-R2 / FE-V / Phase 03–06 supporting evidence。
- 被验证 commit：`c9c19f45d97c2f9b7265c62861ed160a2e987dd5`，branch `main`；`origin/main` 已核对同 SHA。
- 环境：Local Supabase、中央 Account API、两个 Consumer production server、Admin production server、headless Chromium；runner 自动创建并清理随机 fixture。
- 命令与结果：`pnpm --filter admin typecheck` PASS；`pnpm --filter admin build` PASS；`pnpm lint` PASS（Admin MFA 原 `<img>` warning 已清零）；目标文件 `oxfmt --check` PASS；`node --check tests/spikes/e2e/t16-r2-account.mjs` PASS；impeccable detector `[]`；`pnpm run test:e2e:t16-r2` 退出码 0。
- 浏览器断言：T16-R2 汇总 20 项 PASS。目录覆盖 400/403/404/409/412/428/429/500/503；Plans 429/412；Accounts 503/202；Operations 503/429；Subscriptions 412/503；Files 列表 503/删除 202；Policy 412；Platform Settings 412；Origins 503 重试及 late/fresh 乱序响应；既有 Consumer Close/Delete、Admin suspend/restore、网络 unknown、Files 状态/下载/MFA 仍通过。
- 收敛断言：新响应覆盖迟到旧响应；后台刷新失败保留 last-known data；202 显示 accepted 并等待权威刷新；关闭/重开、刷新失败、unknown 状态检查不重发原 mutation。
- 失败保留与复测：第一次运行因平台目录实际统一文案与测试期望不一致而失败（退出码 1，`exerciseAdminErrorCopyMatrix`）；修正测试期望后复跑退出码 0。该失败保留为测试 contract 对齐记录，未删除或放宽断言。
- 未覆盖：FE-V01～16 全量正向高风险流程、逐资源所有状态的完全闭环、390px/完整键盘焦点、Hosted/Staging/生产、真实 Storage 迟到写入及正式发布；继续保持 PARTIAL、NOT_RUN 或 BLOCKED。
...
| 2026-09-10 | T16-R2 / FE-V09 | test | `codex/frontend-plan-r1` | `bb65f1840e8b3f67804a0f8ca3bc36cb9ad4c921` | [GitHub test commit](https://github.com/aisenhub/Aisenhubplatform/commit/bb65f1840e8b3f67804a0f8ca3bc36cb9ad4c921) | PASS | PASS | Files 延迟列表、六类状态/预算边界、DELETE 网络 unknown 与权威检查不自动重发；T16-R2 当前共 18 项 Local 浏览器断言通过 |
...
| 项目 | 事实 |
|---|---|
| Phase 01–08 代码 | 已形成独立代码提交并 push；Phase 08 代码为 `11aa3f2`，当前产品实现基线为 `c9c19f4`。 |
| Local 前端回归 | Admin/Consumer typecheck/build、root lint/typecheck/unit/contracts/docs、T12 五档 70 路由、T16 当前 20 项、M5/Registry 和最终 detector 均已记录 PASS；最新矩阵含资源 generation/epoch、目录 400/403/404/409/412/428/429/500/503、Plans/Accounts/Operations/Subscriptions/Files/Policy/Settings 412/429/503/202 与 Origins 迟到响应收敛。 |
| Git 交付 | FE-D02 代码 commit `54ff79727dd0b7ea734822d23db2c9b58d6fe4ea`、此前 Admin/Files/Settings 增量 commit 与本轮产品+测试 commit `c9c19f45d97c2f9b7265c62861ed160a2e987dd5` 均已推送；文档维护提交 `e5ceb98`、`a974122` 均已推送，当前记录的后续 docs 收口提交由 Git 历史追溯；`origin/main` 已核对一致；未纳入用户已有未跟踪架构文档。 |
...
| P1 | FE-V 状态与故障恢复矩阵 | PARTIAL | `c9c19f4` 已补资源 loader generation/epoch、目录 400/403/404/409/412/428/429/500/503、Plans/Accounts/Operations/Subscriptions/Files/Policy/Settings 的代表性 412/429/503/202、Origins 迟到响应和刷新失败保留已知状态；继续补 FE-V01～16 全量正向高风险 mutation、逐资源完整恢复闭环、390px/键盘焦点与 Hosted 验收。 |
...
1. 继续补 FE-V01～16 的全量正向高风险流程、逐资源完整恢复闭环和 390px/键盘焦点矩阵；本轮 `c9c19f4` 已完成资源 loader generation/epoch、目录完整错误副本、Plans/Accounts/Operations/Subscriptions/Files/Policy/Settings 代表性 412/429/503/202、Origins 迟到响应和关闭/重开/刷新失败状态收敛。
