...
修订日期：2026-09-10；当前代码基线：`main@c9c19f4`（FE-D02 批次边界基线为 `54ff797`，文档维护提交另行记录）。以下是 FE-R1 执行合同与能力边界；Phase 01～08 与 FE-D02 已有本地代码批次和验证记录，但合同级剩余项仍未全部关闭。与旧研究快照描述冲突时，本修订用于执行；公共安全/业务合同仍以上层架构和专题为准。

## 当前执行边界（2026-09-10）

- 本轮已交付代码和代表性 Local 验收；当前按用户安排暂缓 FE-R1 新开发，优先承接其他明确功能。合同仍有效，暂缓不等于剩余验收已关闭。
- 已确认的错误/竞态/状态收敛语义继续作为后续资源页的唯一入口；新功能不得复制第二套 Async、Auth、MFA、错误呈现或 mutation recovery 规则。
- 恢复 FE-R1 时必须从 `main@c9c19f4` 和 VR-0018/VR-0019 开始，补齐 FE-V01～16、逐资源恢复、390px/键盘和托管门槛；不把本轮代表性矩阵称作全量矩阵。
...
已完成核验：原 HTTP create 先生成 codes/receipt，SQL按 creation_operation_id命中已有批次时返回原 metadata，导致重放响应可能携带未入库材料。`54ff797` 已通过独立迁移保存逻辑请求指纹；同参数重放返回 `200 + creation_state=replayed_existing` 及批次元数据，不返回 codes/receipt；异参数返回 `409 IDEMPOTENCY_CONFLICT`。batch create 仍固定 `replay never`，UI 不提供“重发创建以恢复明文”入口；SQL/API 失败用例已保留并复验，`c9c19f4` 另补 Platform Workspace MFA step-up/状态刷新上下文保持、资源 loader generation/epoch、FE-V 代表性 412/429/503/202 和目录错误文案矩阵，Admin MFA 二维码 Next Image 已消除 lint warning。Hosted/完整 FE-V08 尚未运行。
