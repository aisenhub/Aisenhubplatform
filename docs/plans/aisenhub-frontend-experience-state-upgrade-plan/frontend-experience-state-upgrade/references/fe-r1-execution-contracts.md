...
修订日期：2026-09-09；审查代码基线：main@b563a98cb61f17bd666d35a5a1e0f6e9312a21f3。以下是计划修订，不表示产品已实现。前端 Phase 01～08 仍未开始。与旧研究快照描述冲突时，本修订用于执行；公共安全/业务合同仍以上层架构和专题为准。
...
全部初始状态 NOT_RUN。每项记录具体测试文件/用例定位、命令或手工步骤、环境、SHA、结果和脱敏证据；静态检查、本地运行、Staging和生产分列。旧测试改locator要保留原权限/并发/恢复断言，不能只证明中文按钮出现。
