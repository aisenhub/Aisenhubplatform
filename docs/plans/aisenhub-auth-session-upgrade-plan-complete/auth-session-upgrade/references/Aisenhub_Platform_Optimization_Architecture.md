...
以下为现行状态的 UI 映射，不新增 SQL/DTO 枚举；完整迁移及 M4 合同为领域权威：

| 后端事实 | UI 展示 | 动作边界 |
|---|---|---|
| `status=pending` | 等待上传 | 按现有 intent/有效期规则开始 |
| `status=receiving` | 正在接收 | 不重复提交 bytes |
| `status=storing` | 正在保存 | 等待或查询结果 |
| `status=active` | 可用 | 经服务端授权下载/替换/删除 |
| `status=deleting` | 正在删除 | 不显示删除已完成 |
| `status=deleted` | 已删除 | 不提供下载 |
| `write_outcome=unknown` | 正在确认写入结果 | 保留预算，查询/受控恢复，禁止自动重传 |

`unknown` 是 write_outcome，不是 status。failed/blocked/cancel 等展示词仅在对应资源合同确实提供依据时使用；不得从 UI 文案反推新增持久化状态。
...
本参考的 Auth 简图不覆盖并发协议细节；实际执行以本包 master §2.10、frontend-integration-contract 和 ASU-01～05 为准。snapshot 包含独立 stepUp，factor GET 401 先恢复，scoped Cookie/fence/epoch 与 MFA 部分成功均为必要验收。测试贯穿各阶段，不等待“Testing 专题”再补安全用例。
