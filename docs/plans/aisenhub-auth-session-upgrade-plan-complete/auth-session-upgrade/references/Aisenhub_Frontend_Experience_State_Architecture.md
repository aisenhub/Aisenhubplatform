...
> 代码核对基线：`main @ 362db831d49308d0e5ca84965af80d86e944f56c`（2026-09-09）  
...
本文件为 Auth 包内参考，完整 Frontend 独立计划已由用户维护。后续执行前读取 Auth 实际交接；不得把本参考旧 snapshot 列表复制为第二套 runtime。完整 snapshot 包含 resolved 与独立 stepUp，Consumer recent-auth 为邮件验证。普通成功读取不清 stepUp；factor GET 首次 401 先 refresh；MFA 部分成功按标准 details 恢复。

数据保留规则补充：background refresh 保留 last-known data；logout/expired 清理，并通过 epoch 阻止旧异步响应重新填充。显示缓存不是授权缓存。文件 unknown 属于 write_outcome 展示，不能新增 status。完整交互与视觉落地由后续独立计划执行，Auth 本期只验收直接触及的认证界面。
