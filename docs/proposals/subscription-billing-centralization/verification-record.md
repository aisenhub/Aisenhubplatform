...
- 修复后本地模板收到 `POST /api/v1/subscription/checkout` 的 `201`，随后查询接口返回 `200`；staging 已生成一笔 `monthly`、`9.90 CNY`、`pending` 的 Checkout intent，证明服务端付款 URL 创建链路已打通。模板付款入口改为当前标签页可靠跳转，并把 pending Checkout 临时保存在 session storage，返回模板时可继续轮询。真实付款、Afdian Webhook、`query-order` 权威核验和权益结算仍未完成。
