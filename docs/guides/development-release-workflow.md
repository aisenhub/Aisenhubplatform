# Agent 开发、分支合并、Staging 验收与生产发布流程

生效日期：2026-09-14。状态：已接受的工作规则。所有 Agent 开始开发、修复、配置或发布任务前必须阅读；决策依据见 [ADR-0002](../decisions/0002-development-release-environments.md)。规则生效不代表 Hosted 项目、部署流水线、测试或告警已经配置完成。
...
```mermaid
flowchart LR
    L["Local：本地前端 + Supabase Docker"] --> S["Staging：托管前端 + 独立 Hosted Supabase"]
    S --> G["生产前门槛：证据、配置、迁移、恢复方案、批准"]
    G --> P["Production：生产前端 + 独立 Hosted Supabase"]
```

图为标准路线。第 3 节的 R0 不部署应用；R1 允许 Local → 精简生产前门槛 → Production。R2/R3 必须走完整路线。CI 是质量检查，不是第四套业务环境，不要求另建常驻 CI Supabase。

| 对象 | Local | Staging | Production |
| --- | --- | --- | --- |
| Supabase | 本机 Docker；本地数据库、Auth、Storage | 独立 Hosted 项目 | 独立 Hosted 项目 |
| Admin | 本地 Next.js，只接 Local | 固定 HTTPS 测试部署，只接 staging | 生产部署，只接 production |
| 各平台前端/BFF | 本地部署和本地平台凭据 | 各受影响平台的测试部署与 staging Key | 生产部署和生产平台凭据 |
| 数据 | 虚构 fixture，可在确认范围后重置 | 独立测试用户、订单、对象、码与审计 | 真实业务数据 |
| Provider | 默认 Mock | 沙箱或独立测试用途账号；真实交易另行明确 | 正式业务账号 |
| Worker / Webhook | 模拟与本地进程 | 部署真实入口、真实调度，处理测试数据 | 正式入口、调度及生产数据 |

一个中央 staging Supabase 项目服务多个测试平台，不按平台再创建数据库项目。跨平台验收至少使用两用户三平台，并覆盖不同前端 Origin；共享 SDK/BFF 改动需核对所有受影响平台的消费者兼容。未变更的平台可引用同版本基础验收，不能只测参考页就声称所有自定义平台已通过。

Local 前端连接 Hosted staging 属于明确标记的远程联调，不计纯 Local，也不能替代 Hosted 前端验收。Local/Preview 不连接 production。正常 staging 不恢复生产用户或生产备份；生产恢复演练使用另外受控、禁止外发的隔离环境。
...
只维护一条长期 `main` 和短期 `codex/<任务编号或简短名称>` 任务分支，不要求长期develop/staging分支。main是已验收的代码基线，生产运行版本另记准确commit/产物；staging是环境，可以部署任务分支指定commit，不必先合并main。
...
4. R2/R3在任务分支完成Local和staging验收；R0/R1按第3节豁免规则。单人开发不强制第二人审批，但Agent必须实际审查最终diff及测试证据，不能以自己写的代码为由跳过审查。
...
| R1 | 本地目标页面、适用质量检查通过，完整记录staging豁免依据 |
| R2 | Local、适用CI、Hosted staging及审查通过；消费者兼容已验证 |
| R3 | R2条件加上适用的领域合同、权限、迁移升级、并发/恢复和Provider证据通过 |
...
| 级别 | 严格允许范围 | Local 完成后能做什么 | Staging | 生产前门槛 |
| --- | --- | --- | --- | --- |
| R0：不改变运行产物 | 纯文档、注释；仅测试用例且不进入产物、不改变测试工具链或门槛 | 按授权提交/推送代码；不需要部署应用 | 不要求；不是 PASS，记录“范围不适用” | 不部署生产则不适用；若触发站点构建/运行部署，重新分级 |
| R1：纯展示 | 非业务静态文案、图片、局部颜色/间距，满足下方全部条件 | 可跳过 staging；完成精简门槛并已有发布授权后部署 | 可豁免，必须记录证据和理由 | G0–G5 精简检查；发布后 G6 |
| R2：一般功能 | 无 R3 影响的前端交互、搜索筛选分页、共享 UI、可访问性交互、构建/依赖/CI 配置等 | 本地通过后准备或在授权内部署 staging | 必须：受影响真实 Hosted 链路 | G0–G5 完整适用项；发布后 G6 |
| R3：核心与安全 | 数据库、Auth、权限、支付、权益、兑换、Storage、业务 API/SDK/BFF、Secret、Worker、恢复等 | 本地通过不能直接上线；必须 staging 和领域验收 | 必须：集成、负向、并发/恢复及适用 Provider 验收 | G0–G5 + 领域门槛；发布后 G6 |
...
| 普通页面交互/列表筛选 | apps/admin、apps/template-preview、packages/ui | R2 | Hosted UI、键盘、窄屏、空/错/加载；牵涉作用域或写操作升 R3 |
...
- 先确定用户/业务政策再实现有争议规则。没有 Provider 账号、staging 或决策时可完成本地准备，记录 BLOCKED，不把功能标端到端完成。
...
| Worker | `pnpm test:maintenance` | handler测试，不证明Hosted cron实际运行 |
...
| 备份恢复 | `pnpm test:ops:m6-02-local` | 本地演练，不能外推Hosted备份/Storage恢复成功 |

Hosted 测试只能使用为目标环境核验过的安全 runner/操作清单；本地脚本若含 reset、固定端点或清理假设，不能仅替换 URL 后指向 staging。缺少 Hosted runner 时先完善测试方案和隔离检查，确需新增脚本则在任务范围内实施，未执行为 NOT_RUN。

### 4.2 CI 的实际边界

当前 [Workflow](../../.github/workflows/workflow.yml)执行格式、lint、typecheck、build、单测、运行时及文档合同检查，未覆盖全部 SQL、实际 API、浏览器、Hosted 或部署流程。CI不是staging替代品，也不因本规则自动获得部署能力。

正式发布记录准确候选版本的CI结果；尚未触发CI时记录NOT_RUN，可继续本地或已授权staging准备，但不能宣称CI通过。无可用CI时，仅在用户明确接受本次替代方案后，用可复核的干净环境执行等价质量检查；不能悄悄省略失败项。纯本地开发交接不要求为了得到CI结果自动推送。

## 5. Staging 部署与验收

### 5.1 部署前

1. 确认已有 staging 部署授权及准确项目/前端目标；用户已明确授权同一范围时不重复询问。只授权开发不等于授权改远程Secret、建项目或付费。
2. 固定候选 commit、lockfile、迁移清单/hash、SDK/Registry版本和构建输入；记录现有staging版本、数据范围、恢复点与其他测试所有者。
3. 确认staging与production的Auth/DB/Storage/Key/Provider/回调/调度目标分离；测试账号、邮件接收人、金额上限明确。
4. 按第 8 节顺序部署迁移、API/Worker和前端。共享staging每次只保留一个明确验收候选；其他部署覆盖后旧证据不可直接沿用。

### 5.2 Staging数据保留、批次清理与重建

Staging默认持续保留数据，不随部署重置。普通部署和UI修改沿用原有数据；数据库迁移必须在已有数据上验证升级。全新安装测试优先使用Local临时数据库或专门隔离实例，不为获得干净测试结果清空共享staging。

| 数据类别 | 处理规则 |
| --- | --- |
| 长期基础数据 | 测试平台、专用测试用户/管理员、套餐及Provider mapping持续保留；通过既定管理流程维护，不随每次测试重建 |
| 临时测试数据 | 每次测试使用独立批次标识或受控清单记录归属；验收结束后只清理该批次中可安全删除的数据，不影响其他批次 |
| 验收证据数据 | 订单、Settlement、Grant、Webhook、Job、兑换记录和审计保留至验收及后续处理完成，并满足事先明确的保留期限；再按受控保留/清理流程处理 |

批次标识可放在测试清单中，不要求为了追踪批次自行增加业务字段或修改数据库。清单记录任务、环境、创建时间、对象ID/范围、负责人、保留期限和清理状态，不包含真实兑换码、Secret或用户数据。保留期限未明确时先保留并报告，不由Agent猜测删除日期；容量不足先盘点可清理批次，不能自动清空。

每次批次清理必须按以下顺序执行：

1. 核对staging项目、批次所有权、实际记录和关联对象，列出拟处理范围、数量及保留项；确认在已授权测试清理范围内，已有明确授权不重复询问。
2. 确认没有未完成订单、待重投Webhook、运行中/待恢复Job、退款、交付或人工复核。小额真实测试付款仍是外部真实交易，删除本地数据不会取消付款或退款；未完成外部事项时保留对应绑定和完整证据链。
3. 保存必要的脱敏验收及失败证据，通过已有授权管理/领域清理入口处理。普通fixture清理不授权硬删账本、审计或绕过外键；入口不存在时先制定独立清理任务，不执行宽范围SQL删除。
4. 按现有文件生命周期协调数据库与实际Storage对象，清理专属Auth测试用户前核对其账户、订单和对象依赖；不能只删一边，也不能删除共享基础用户。
5. 核对实际处理数量、残留、对象一致性及其他批次未受影响，记录结果和失败恢复位置。清理失败保留证据并报告，不通过删除失败记录让测试看起来通过。

全库重建是独立、破坏性操作，必须有明确授权，不能从部署、测试或Git自动合并授权推导。执行前交付可审阅的目标、原因、数据范围、备份/证据、未完成外部交易处置、调度与入站隔离、恢复方案。不能为重建擅自丢弃迟到回调或重投任务；无法保证安全接收和恢复时阻塞重建。

重建后重新验证迁移、基础fixture、角色权限、Auth/Storage、Secret引用、Provider绑定和调度，并保留足以防止旧外部事件重复授予的交易去重依据。重建改变数据与配置基线，受影响的staging验收必须重跑，旧PASS不自动沿用。

### 5.3 必测矩阵

所有 R2/R3 执行共同项，再执行所有受影响领域行；不要求纯列表样式变更重复真实退款，但不能省略其实际影响。每项明确 Local/Mock/Hosted/Provider 范围。

| 领域 | Hosted 验收内容 | 通过依据 |
| --- | --- | --- |
| 共同项 | 实际构建启动、HTTPS、API/BFF路径、环境匹配、请求错误、日志脱敏、旧消费者兼容 | 目标版本可识别；完整调用成功和失败均符合合同 |
| Auth与安全 | 登录/退出/刷新/回调/邮件、过期与撤销、MFA、Origin/CSRF、错误Key/JWT | 未授权/跨环境请求拒绝；不靠关闭MFA或扩大allowlist通过 |
| SQL与隔离 | 托管迁移升级、executor/owner/search_path/RLS、两用户三平台、同名Plan | 跨平台/账户写入失败，合法操作成功；不使用超级用户代替业务角色 |
| Storage | 上传下载/替换/删除、私有权限、大小/配额、网络失败和重试 | DB及实际对象一致；错误权限无读取；未知结果可恢复 |
| Checkout与支付 | 价格/币种/Plan/SKU/期限/version、双击、跨Tab、断响应、迟到付款 | 同意图恢复；钱款/Order/Settlement/Grant关系可核对；不错误授予 |
| Webhook/Provider | 签名、重放、无效字段、权威查询、超时限流、漏通知补单 | ACK后任务持久；重放无重复Grant；未确认事实不当成功 |
...
| Worker/cron | Vault/Token、完整网关路径、真实触发、业务结果、停机/重叠/超时接管 | 至少连续2轮调度，并含1次有任务执行和1次故障恢复；只有空队列200不合格 |
| 告警与恢复 | 失败告警实际送达、恢复通知、备份DB+对象、墓碑和积压重放 | 接收人确认；隔离恢复记录、RPO/RTO实测；不把备份开关当可恢复证明 |

staging验收后修复了代码、迁移、核心配置或依赖，必须重跑受影响用例及共同链路。文档说明修正可按影响记录不重跑业务；不能拿旧commit的PASS覆盖新版本。容量比生产小的差异必须记录，不能外推生产性能。
...
| 门槛 | R1精简要求 | R2/R3完整要求 |
| --- | --- | --- |
| G0 范围与版本 | 核对整次产物符合R1，没有夹带高风险改动 | 列完整差异、受影响平台/API/迁移/配置/依赖；候选与staging验收一致 |
| G1 测试证据 | 适用静态/构建、目标页面检查及CI；书面记录staging豁免理由 | Local、CI、Hosted所有适用项有证据；领域门槛通过；必需项无FAIL/NOT_RUN/PARTIAL/BLOCKED |
...
G5不是每一步重复要确认：已有授权精确覆盖该版本、环境和动作时记录来源并继续；超出范围才请求补充。申请发布批准前，Agent应完成所有已授权、可完成的本地/staging准备，交付具体可审阅的版本、证据、配置差异和回退清单。

门槛有效性绑定代码、迁移、配置版本、目标环境及授权范围。批准后候选变化、环境漂移、备份失效或出现新关键失败，重新检查受影响门槛；批准不能永久沿用。支付、身份、跨平台泄露或数据损坏类未解决关键风险不得由Agent自行延期上线。

“生产前验证”以只读预检及staging证据为主，不提前在生产执行测试迁移或试退款。首次部署可先保持购买/敏感功能关闭，在批准的受控验证通过后再批准或按已有范围开启；生产观察是发布后步骤，不循环要求“上线前已完成上线后观察”。
...
- staging测试订单/用户/Grant/兑换码必须与生产隔离。没有独立Provider测试能力时先Mock；真实渠道合同缺口标BLOCKED，不能冒充已验收。
- 若必须使用正式渠道账号做受控测试，需明确授权、专用商品/订单范围、回调隔离、金额/次数上限和退款安排。不得把正式账号全局Webhook改指staging，也不得让staging扫描/结算真实客户订单；隔离无法保证则停止。
...
- 共享领域过程仍是唯一业务写入口；不得为了staging通过改成高权限直写或关闭RLS。
...
2. 同一份迁移从Local验证到staging，再到production；逐环境核对migration history/hash及实际schema漂移。环境已经应用的迁移不要改内容后继续冒充同版本。迁移中不得写真实Secret或staging订单/用户。
...
5. 候选源码、锁文件、依赖及迁移保持一致。Next.js公开环境值可能在构建时内联；不得直接把内含staging地址的bundle晋升生产。需要分环境重建时，用同一源码/锁文件/构建方式，记录允许差异并扫描生产包，额外验证配置绑定。
6. 同步配置名称与模板，但Secret各环境独立生成。Secret轮换先在staging演练流程，生产值仅在批准操作中核验/切换，禁止复制生产密钥到staging。
...
| 资产/边界 | 必须做到 |
| --- | --- |
| Supabase项目 | Local、staging、production各自实例；部署前校验project ref与目标域名，不用生产库内一个environment列冒充隔离 |
| Auth | 独立用户/会话/管理员MFA；精确回调白名单，邮件限制测试接收人；生产JWT在staging、staging JWT在生产均拒绝 |
| 浏览器与BFF | 独立Origin和Cookie边界，避免父域共享Cookie；公开配置成套对应同环境；staging显著标识，不降低服务端校验 |
| 平台凭据 | 同名平台也使用独立Key/HMAC和scope；Key跨环境请求拒绝；server-only，不进入NEXT_PUBLIC或浏览器包 |
| 数据/Storage/码 | 独立fixture、bucket对象和交付；不复制生产用户/订单/码；恢复演练与日常staging分开 |
| Provider | 独立账号/商品/凭据/回调或经证明的受控隔离；staging不得处理生产订单；公网路径Secret不等于完整环境隔离 |
| Worker/Vault | URL、Token、worker身份、cron和告警接收目标均核验；任务恢复前先检查环境；禁用staging后仍需处理未完成真实测试交易 |
...
- R2：加测受影响Hosted链路、登录/页面/错误率和消费者兼容；R3再核对DB、Webhook、Job、Grant/审计、配置与告警。
...
生产已部署与生产观察完成分开记录。修复或回退后重验受影响链路，不抹除原失败。紧急安全事件默认先使用既定止损/回退方案；紧急新代码仍按R3，确实无法staging时只能由用户明确批准例外、接受风险并要求后续补验，Agent不能自行豁免。
...
CI：候选SHA、结果或明确替代批准：
Staging：目标、部署版本、用例、结果；豁免仅R0/R1并写理由：
...
证据放实际任务回复、CI/测试平台或已有proposal验证记录；不要在当前架构中追加执行流水。没有staging或所需测试时，报告“本地完成，Hosted/发布阻塞”，不伪造PASS、不降低权限、不删除测试。用户只授权某一任务时，不自动继续下一阶段。

## 12. 规则维护和当前边界

本文件不自动创建环境、不修改CI配置、不执行迁移、不分配Secret、不收费、不部署。云环境是否存在/可用以及免费项目是否暂停，执行前以真实状态核验。首次环境配置尚未验收时R2/R3不能跳过staging；R0/R1仍按其独立范围处理。
