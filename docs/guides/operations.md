...
关闭账户的资料/文件不会再由代码猜测固定天数：`private.account_retention_policy` 由法务/财务确认后写入个人信息保留天数、来源和依据；未配置时匿名化任务失败关闭。到期清理只匿名化个人字段和可识别审计字段，保留最小账务/审计事实、订单、金额、结算、Grant 与快照。备份最多保留30天并隔离访问是备份策略，不等同个人信息保留期；恢复必须重应用删除墓碑。若保留要求阻止清除，任务显式blocked，不返回删除完成。

订阅生命周期：Admin 的“暂停新购买”只阻止新的付费 Checkout，不停发已有有限期权益；已付款订单继续进入结算/对账。Plan 归档允许保留历史映射和快照，但归档后的旧 Plan 不可修改，新的 Checkout 必须失败关闭；变更商业条款应创建新 Plan。
