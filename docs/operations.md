...
本文件从属于 [架构基线](architecture.md)。以下是必须实现和演练的合同，不表示当前仓库已有运行环境、备份或通过的测试。
...
CI依次执行格式/lint/typecheck、领域单测、空库reset、上一版本upgrade、角色权限和pgTAP、Account攻击测试、并发与故障注入、Admin/模板build、Registry校验及真实消费项目E2E。测试期间仅使用虚构数据。
...
清理/删除重试每分钟运行，对象对账每小时分批运行，联合备份每天运行。任务用数据库job lease、fencing_token、retry_count和next_attempt_at，禁止将Edge响应后的未跟踪Promise作为可靠任务。
...
验收报告必须分清“文档静态检查”“自动化测试”“Staging演练”“生产观察”。没有实现时只能完成第一类。
