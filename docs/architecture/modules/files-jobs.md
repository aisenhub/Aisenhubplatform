...
本文件描述当前文件、Storage 和持久任务实现，属于 [系统架构](../overview.md) 的文件模块。文件不解析、不执行、不预览、不解压，扩展名不限。单一私有 bucket 为 platform-config-files，路径为 platform_id/platform_account_id/file_id；原始文件名仅作为 metadata，不进入路径。
...
本节定义文件 API、受控 SQL 入口和备份屏障之间的公共边界。具体表、函数和任务以当前 migrations/functions 为准；HTTP 层不能自行拼接第二套配额或删除算法。
...
| `file_write_attempt_mark_unknown` / `file_write_attempt_finalize` | account/job recovery executor；trusted adapter结果、provider request id、size/hash | unknown 或 confirmed 后推进 storing 状态，记录证据和 Audit | in_flight/unknown不得由客户端改写；旧fence、错误file或重复矛盾结果拒绝；unknown持续占用 |
