...
| supabase/functions/maintenance | 文件清理、对账、保留清理、身份删除和计费任务 lease/fence 步骤；入口按 files / identity / billing capability token 隔离 |
...
Admin API 与 Account API 在同一 Edge 源文件中分派，分别切换数据库 executor 角色。管理端资源请求由 Next.js 按 Admin OpenAPI 对齐的显式 method/path allowlist 转发到该服务，并为上游请求设置 deadline；管理端本身不直接连接 SQL。Consumer BFF 同样保持显式 allowlist 与上游 deadline。
