...
### 当前平台端页面范围（2026-09-11）

这里的“平台端”指 Aisenhub 自有平台给平台用户使用的页面，不是要求用户把自己的产品接入 Aisenhub 的 Consumer 集成模板。`apps/template-preview` 当前是本地参考页面与 API 接入示例，不能据此宣称生产部署、完整后端链路或外部用户应用集成已经完成。

| 页面 | 当前范围 | 明确边界 |
|---|---|---|
| `/account` | 账户设置：用户名、已有邮箱/手机号展示；未绑定时通过 Supabase Auth 获取验证码并绑定；验证码面板只在点击后出现，60 秒倒计时；修改密码先校验旧密码 | 个人简介已移除；页面验证依赖公共 Supabase 环境变量，不能把参考页面当作生产账户服务验收 |
| `/files` | 配置文件列表、上传、删除和状态展示；根路径 `/` 重定向到此页 | 当前是本地演示交互；生产上传、权限、配额和 Storage 状态仍以 Account API / 文件专题合同为准 |
| `/subscription` | 固定四档：免费版 ¥0、月订阅 ¥9.9、年订阅 ¥19.9、永久订阅 ¥29.9；激活码格式先校验，再进入后台验证占位流程 | 月/年展示爱发电付款等待区并锁定套餐；永久订阅只弹出联系管理员获取激活码提示；支付回调和兑换后端尚未在该参考页接通 |
| 主导航 | 仅保留账户设置、配置文件、订阅方案 | 总览页已移除，不再作为平台端功能入口 |

当前页面改造只改变平台端参考体验和文档口径，不新增组织、支付领域、第三方用户集成或跨平台业务能力。若需要真正的生产接口或外部应用接入，应新建独立任务，不从本参考页的静态/本地交互推定已实现。
...
历史文档修订曾包含 ASU-01～05 与 ASU-V01～16；对应计划包已从当前仓库清理。产品实施状态、运行验收和后续任务不由这段历史摘要自动升级，当前以实际代码和仍保留的专题文档为准。
