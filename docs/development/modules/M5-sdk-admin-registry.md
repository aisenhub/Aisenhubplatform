...
状态：IN_PROGRESS；M5-02已完成Local tarball打包、浏览器/服务端边界、独立消费者类型检查和Node/Edge导入验收；M5-03 Local Admin资源导航、审计、服务端筛选、状态动作和Replace/purge blocked原因展示已完成。2026-09-11按产品决策将`apps/template-preview`收敛为 Aisenhub 平台端公开页面参考与 API 接入示例，不再把它定位为要求下游用户集成的完整 Consumer 应用；当前页面仍可保留账户设置中的 Supabase Auth 验证参考，但不宣称完整登录、用户状态同步、真实Account API/BFF或生产结算已完成。旧M5-04/M5-05中的完整Auth Consumer范围属于历史验证，需按新模板范围重新基线。Hosted联合验收与M5-06正式发布仍未完成。依赖M2～M4合同与纵向链路，完成一致性、交互、打包和复用验收。DP2已列出[M5-01～06任务与资源覆盖表](../roadmap-dp2.md)，盘点可提前，M4-10后冻结文件集成细节。

## 1. 范围

将account-auth、account-auth-nextjs、account-server形成正式版本产物；完善Admin资源管理与 Aisenhub 平台端参考页面；建立可独立安装的页面示例。MakerKit/shadcn仅作为工程结构和组件组织参考，不作为视觉模板；UI独立评审操作顺畅度、信息清晰度、状态反馈及时性和页面质感。
...
统一品牌配置name/logo/routes；`apps/template-preview`只包含 Aisenhub 平台端 UI、页面结构和 API 接入示例，不把自己定义成下游用户应用。当前页面包括账户设置、配置文件、订阅方案和激活码参考交互；账户页可以用公共 Supabase 环境变量演示邮箱/手机号验证码绑定与旧密码校验，但不宣称完整登录、Cookie/session、用户状态同步、真实Account API、Storage或生产订阅结算已完成。需要生产认证或账户能力的平台，单独选择 SDK 并按合同接入。

Registry构建固定CLI版本，记录兼容SDK范围、实际构建版本及校验和；原始版本URL不可变。复制UI后允许产品定制，不复制领域计算。

参考页面包含账户设置、配置文件、订阅方案、激活码输入、空数据/示例数据、表单反馈、资源列表和 API 代码片段；页面中的远程验证或支付跳转必须标注为参考/占位状态，不把演示数据描述成真实权益、账户状态或结算结果。

## 5. 真实消费测试与发布

CI新建独立Next项目，安装本次pack的三个SDK与Registry，不使用monorepo workspace链接替代；连接隔离测试后端，创建第二Platform，仅配置即可接入。

完成V-SDK-01/02、V-UI-01及当前平台端页面回归；typecheck/build覆盖账户设置、配置文件、订阅和激活码参考页面，验证页面不把参考交互伪装成生产用户集成或结算结果。扫描浏览器bundle和打包文件，证明不含后端Secret或伪装成NEXT_PUBLIC的配置。旧 V-INTEGRATION-01 仅作为历史 Consumer 验收记录，不作为本范围默认门槛。
