...
本文件定义用例ID与完成证据。已有任务报告中的Local及Staging基础探针结果保留；当前Gate以[进度](status.md)和[DP2校准](evidence/DP2-baseline.md)为准，不能把历史部分PASS扩展为全部通过。实现任务引用ID，报告必须绑定commit与真实环境。
...
| G5-L | 新项目安装本地实际打包SDK与Registry并全链路E2E，Secret扫描通过 | 不代表产物已发布到正式地址 |
| G5-P | 已授权向正式scope/Registry发布固定版本，并从正式地址安装验证 | 不代表生产后端部署已授权 |
| G6 | 配置确认、联合恢复、轮换、性能、上线/恢复清单通过 | 仍需实际发布任务和对应授权 |

M3/M4可在G2-L后开展本地实现，但托管集成结论依赖G2-S。不得用生产环境完成首次破坏性实验。

DP2执行映射：T18-L确认G1/G2-L，T18-S确认G0-S/G2-S，M3-R1核对G3，M4-10/11分别确认G4-L/G4-S，M5-05/06分别确认G5-L/G5-P，M6-05确认G6。任务见[收尾清单](tasks/closeout-01.md)、[第三批](tasks/batch-03.md)和[路线](roadmap-dp2.md)。目前根test:api/test:e2e仍占位，CI也未覆盖SQL/API/浏览器；T16-R2补齐后才可把这些入口作为验收证据。
...
| V-UI-01 | Admin敏感操作服务端校验；Profile冲突、批次确认、文件剩余预算UX正确；UI以操作顺畅、信息清楚、状态反馈及时、页面有质感验收，不要求复刻MakerKit视觉 |
